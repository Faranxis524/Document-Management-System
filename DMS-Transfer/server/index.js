...
app.put('/records/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const newStatus = db.calculateStatus({ 
      ...record, 
      ...payload, 
      dateSent: payload.dateSent || record.dateSent,
      targetDate: payload.targetDate || record.targetDate,
      actionTaken: payload.actionTaken || record.actionTaken
    });
...
app.post('/export/csv', authMiddleware, asyncHandler(async (req, res) => {
  ...
  const headers = [
    ...
    'Concerned Units', 'Date Sent', 'Status'
  ];
...
app.get('/dashboard/summary', authMiddleware, asyncHandler(async (req, res) => {
  const filters = {};
  if (req.user.role === 'SECTION') {
    filters.section = req.user.section;
  }
  ...
  allRecords.forEach(record => {
    ...
    const status = record.status || db.calculateStatus(record);
    ...
    if (record.dateSent) {
      const dateSent = new Date(record.dateSent + 'T00:00:00');
      if (dateSent.getMonth() === currentMonth && dateSent.getFullYear() === currentYear) {
        stats.completedThisMonth++;
      }
    }
    
    // Update section stats
    if (record.section && stats.sections[record.section]) {
      stats.sections[record.section].total++;
      if (status === 'Pending') stats.sections[record.section].pending++;
      if (status === 'Ongoing') stats.sections[record.section].ongoing++;
      if (status === 'Completed') stats.sections[record.section].completed++;
      if (status === 'Overdue') stats.sections[record.section].overdue++;
    }
  });
...
async function startServer() {
  ...
  server = app.listen(port, host, () => {
    ...
    console.log(`Access from network: http://192.168.254.193:${port}`);
  });
