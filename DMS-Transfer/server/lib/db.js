...
async function getNextCounter({ scope, section, dateReceived }) {
  ...
  // Next number is based on the highest existing sequence for that date.
  if (dateReceived) {
    if (isSqlite) {
      const existing = await getSqlite('SELECT * FROM counters WHERE scope = ? AND section IS ?;', [scope, section]);
      const highestActual = await getHighestActualSequenceForCounter({ scope, section, dateReceived });
  ...
  if (isSqlite) {
    return await allSqlite(
      'SELECT al.*, r.section AS section, r.sectionCtrlNo AS sectionCtrlNo, r.mcCtrlNo AS mcCtrlNo ' +
        ...
        'WHERE al.recordId = ? ORDER BY al.performedAt DESC;',
      [recordId]
    );
  }
  const result = await pgPool.query(
    'SELECT al.*, r.section AS section, r.sectionctrlno AS "sectionCtrlNo", r.mcctrlno AS "mcCtrlNo" ' +
      'FROM audit_logs al ' +
      'LEFT JOIN records r ON r.id = al.recordId ' +
      'WHERE al.recordId = $1 ORDER BY al.performedAt DESC;',
    [recordId]
  );
  return result.rows;
}
